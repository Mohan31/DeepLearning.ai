Toggle navigationNavigation openNavigation closed

[![Coursera](data:image/svg+xml;base64,)](https://www.coursera.org/)

![Improving Deep Neural Networks: Hyperparameter tuning, Regularization
and
Optimization](https://d3njjcbhbojbot.cloudfront.net/api/utilities/v1/imageproxy/https://coursera-course-photos.s3.amazonaws.com/47/e11e60789711e794f6d93ac1342f51/deeplearningai.png?auto=format%2Ccompress&dpr=1&w=128&h=32)

[Catalog](https://www.coursera.org/browse)

``` {aria-hidden="true" style="position: absolute; visibility: hidden; white-space: pre; font-family: OpenSans, Arial, sans-serif; font-size: 14px; font-style: normal; font-variant: normal; font-weight: 400; word-spacing: 0px; letter-spacing: 0px; text-indent: 0px; text-rendering: auto; text-transform: none;"}
```

**

-   [Browse](https://www.coursera.org/browse)
-   [Search](https://www.coursera.org/courses)
-   [For Enterprise](https://www.coursera.org/enterprise)
-   [Profile](https://www.coursera.org/account-profile)
-   [My Purchases](https://www.coursera.org/my-purchases)
-   [Settings](https://www.coursera.org/account-settings)
-   [Help Center](https://learner.coursera.help/)
-   Log Out
-   user
    Raheja**

**Back to Week 2

**Lessons

[Prev](https://www.coursera.org/learn/deep-neural-network/lecture/RFANA/the-problem-of-local-optima)

[Next](https://www.coursera.org/learn/deep-neural-network/notebook/eNLYh/optimization)

-   ### Optimization algorithms {.lesson-name .headline-1-text .color-primary-text}

-   ### Practice Questions {.lesson-name .headline-1-text .color-primary-text}

    -   [](https://www.coursera.org/learn/deep-neural-network/exam/rqT6n/optimization-algorithms)

        #####  {.item-name .inline-child .caption-text}

        **Quiz:**

        Optimization algorithms

        10 questions

-   ### Programming assignment {.lesson-name .headline-1-text .color-primary-text}

    -   [](https://www.coursera.org/learn/deep-neural-network/notebook/eNLYh/optimization)

        **

        #####  {.item-name .inline-child .caption-text}

        Optimization

        2h

    -   [](https://www.coursera.org/learn/deep-neural-network/programming/Ckiv2/optimization)

        **

        #####  {.item-name .inline-child .caption-text}

        **Programming Assignment:**

        Optimization

        30 min

-   ### Heroes of Deep Learning (Optional) {.lesson-name .headline-1-text .color-primary-text}

QUIZ

Optimization algorithms

10 questions

Your Score

9/10 points (90%)

We keep your highest score.

View Latest Submission

Take it again

**

**

**

\<img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=946401778754875&amp;ev=PageView&amp;noscript=1"\>

![](./quiz1_files/0)

### Confirm Navigation

Are you sure you want to leave this page?

Stay on this Page

  

Leave this Page

### Confirm Navigation

Are you sure you want to leave this page?

Stay on this Page

  

Leave this Page


